...
evaluate :: Expr -> Maybe Int
evaluate (Num n) = Just n
evaluate (Logical b) = Just (fromEnum b)
evaluate (Add e1 e2)
    | evaluate e1 == Nothing || evaluate e2 == Nothing = Nothing
    | otherwise = Just (x + y)
    where
    (Just x, Just y) = (evaluate e1, evaluate e2)
evaluate (Div e1 e2)
    | evaluate e2 == Just 0 = Nothing
    | otherwise = Just (x `div` y)
    where
    (Just x, Just y) = (evaluate e1, evaluate e2)
evaluate (Less e1 e2)
    | isLessExpr e1 e2 = Just (fromEnum $ evaluate e1 < evaluate e2)
    | otherwise = Nothing
evaluate (If e1 e2 e3)
    | evaluateBool e1 == Just True = evaluate e2
    | evaluateBool e1 == Nothing = Nothing
    | otherwise = evaluate e3

isLessExpr :: Expr -> Expr -> Bool
isLessExpr (Less _ _) _ = True
isLessExpr _ (Less _ _) = True
isLessExpr _ _ = False

evaluateBool :: Expr -> Maybe Bool
evaluateBool (Less e1 e2) = Just (evaluate e1 < evaluate e2)
evaluateBool (Logical b) = Just b
