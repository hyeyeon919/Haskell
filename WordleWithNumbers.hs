...
Each round of the game proceeds as follows:
    ...
    2. The computer outputs a tuple of two lists of Ints, (strongMatch, weakMatch).
...
When called, solve should do the following:
    1. Read an Int N from the terminal, indicating the length of the sequence to be guessed.
    2. In each round of the game, output a sequence of N numbers, each in the range [0..9],
       and read the computer's response (a tuple of two lists of Ints).
...
--n을 입력받는다.
--n의 길이인 모두 0인 리스트를 넣는다.
--결과에 stringMatch에 있는 원소들만 고정한다.
--n의 길이인 모두 1인 리스트를 넣는다.
result = []
solve :: IO ()
solve = do
    myVar <- readLn :: IO Int
    print $ makeList myVar 0
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result0 = concatResult result strongMatch

    print $ makeList myVar 1
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result1 = concatResult result0 strongMatch

    print $ makeList myVar 2
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result2 = concatResult result1 strongMatch

    print $ makeList myVar 3
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result3 = concatResult result2 strongMatch

    print $ makeList myVar 4
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result4 = concatResult result3 strongMatch

    print $ makeList myVar 5
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result5 = concatResult result4 strongMatch

    print $ makeList myVar 6
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result6 = concatResult result5 strongMatch

    print $ makeList myVar 7
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result7 = concatResult result6 strongMatch

    print $ makeList myVar 8
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result8 = concatResult result7 strongMatch

    print $ makeList myVar 9
    (strongMatch, weakMatch) <- readLn :: IO ([Int],[Int])
    let result9 = concatResult result8 strongMatch
--
    let list = [0..9]
    let start = makeList myVar 0
    let final = recursive list result9 start
    print $ take myVar final

recursive :: [Int] -> [[Int]] -> [Int] -> [Int]
recursive [] _ start = start
recursive (i:is) (r:rs) start = recursive is rs newStart
  where
    newStart = indexStrongMatch i r start

concatResult :: [[Int]] -> [Int] -> [[Int]]
concatResult x y = x ++ [y]

indexStrongMatch :: Int -> [Int] -> [Int] -> [Int]
indexStrongMatch _ [] list = list
indexStrongMatch x (i:is) list = indexStrongMatch x is (insertAtIndex i x list)

--리스트 입력받기
--리스트 입력받기
insertAtIndex :: Int -> Int -> [Int] -> [Int]
insertAtIndex i element list =
  take i list ++ [element] ++ drop (i + 1) list


makeList :: Int -> Int -> [Int]
makeList n guess = replicate n guess
